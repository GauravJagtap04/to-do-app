...
const completedOutputFldLst = document.getElementById(
  "output-field-list-completed"
);

const taskData = JSON.parse(localStorage.getItem("data")) || [];
const completedTaskData =
  JSON.parse(localStorage.getItem("completed_data")) || [];
...
const updateOutputFldLst = () => {
  ...
  taskData.forEach(({ id, task, priority, date, minutes }) => {
    let priorityColor;
...
const updateCompletedOutputFldLst = () => {
  ...
  completedTaskData.forEach(({ id, task, priority, date, minutes }) => {
    let priorityColor;
    if (priority === "L") priorityColor = "var(--low-priority)";
    else if (priority === "M") priorityColor = "var(--med-priority)";
    else if (priority === "H") priorityColor = "var(--high-priority)";
    ...
    completedOutputFldLst.innerHTML += `
                <li id="${id}">
					<table style="box-shadow: 0px 0px 10px ${priorityColor};">
						<tr>
							<td style="width: 95%; text-align: center; margin: 0; padding: 0;" title="${
                date + " " + minutes
              }">${task}</td>
